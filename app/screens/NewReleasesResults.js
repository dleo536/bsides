...
import { searchNewAlbums } from "../api/SpotifyAPI";
...
const NewReleasesResults = () => {
  ...
  const market = route?.params?.market || "US";
  ...
  useEffect(() => {
    navigation.setOptions({ title: "New Releases" });
  }, [navigation]);
  ...
  const fetchAlbums = useCallback(
    async (nextOffset = 0, reset = false) => {
      ...
      setError(null);
      try {
        const response = await searchNewAlbums({
          limit: PAGE_SIZE,
          offset: nextOffset,
          market,
        });
        ...
        console.error("NewReleasesResults fetch error:", fetchError);
        setError("Could not load new releases.");
      } finally {
        setLoading(false);
      }
    },
    [hasMore, loading, market]
  );
...
export default NewReleasesResults;
