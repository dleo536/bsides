...
import { searchNewAlbumsByMarket } from "../api/SpotifyAPI";
...
const CountryResults = () => {
  ...
  const countryName = route?.params?.countryName || "United States";
  ...
  useEffect(() => {
    navigation.setOptions({ title: `New in ${countryName}` });
  ...
  const fetchAlbums = useCallback(
    async (nextOffset = 0, reset = false) => {
      ...
      setError(null);
      try {
        const response = await searchNewAlbumsByMarket(countryCode, {
          limit: PAGE_SIZE,
          offset: nextOffset,
        });
        const items = Array.isArray(response?.items) ? response.items : [];

        setAlbums((prev) => (reset ? items : [...prev, ...items]));
        setOffset(response?.nextOffset ?? nextOffset + items.length);
        setHasMore(Boolean(response?.hasMore));
      } catch (fetchError) {
        console.error("CountryResults fetch error:", fetchError);
        setError(`Could not load New in ${countryName}.`);
      } finally {
        setLoading(false);
      }
    },
    [countryCode, countryName, hasMore, loading]
  );
  ...
  const emptyState = useMemo(() => {
    ...
    return (
      <View style={styles.centerState}>
        <Text style={styles.emptyText}>No results available right now.</Text>
      </View>
    );
